...
lists), `id/<name>/song.json` (the shape; its `essay` field points at the words), and that essay.
No feedback.md? Stop and say: run `/cyborge-feedback <name>` first.
...
- shape changed → `python3 tools/song/build.py id/<name>/song.json --index id/<name>/index.html`.
